...
handle_help() {
	...
	"list backlinks")
        ...
        Or Like This [Link Text](denote:20231206T114112)
        "
		;;
...
handle_add_tag() {
	...
	process_files_from_stdin_or_args "$@" | while read -r file; do
...
handle_list_notes_for_tags() {
	local tags="$1"
	shift
	for file in "${DENOTE_MD_NOTES_PATH}"*.md; do
		if test "$tags" = ""; then
			print_note_name "$file"
			continue
		fi
		if filename_matches_tags "$file" "$tags"; then
			print_note_name "$file"
		fi
	done
}
...
directory_from_filename() {
	...
	file="$1"
	maybe_folder="${file%/*}"
	if test "${file}" = "${maybe_folder}"; then
		echo ""
		return
	fi
	folder="${maybe_folder}"
...
filename_matches_tags() {
	...
	for tag in ${tags//,/ }; do
		...
		if test "${tag:0:1}" = "-"; then
			# This is an exclusion tag, remove the leading hyphen
			tag="${tag#-}"
			if filename_has_tag "$file" "$tag"; then
				return 1
			fi
		else
...
print_note_name() {
	IFS=$'\n' read -r -d '' -a lines <<<"$@"
	for input_string in "${lines[@]}"; do
		if test "$enable_colors" = "false"; then
		...
		local identifier
		if [[ $input_string =~ $identifier_regex ]]; then
